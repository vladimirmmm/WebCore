    ...
    if (element != null)
    {
        ...
        if (!IsNull(fieldname)) {
            if (element.tagName == "INPUT"
                ...
                return { Key: fieldname, Value: (<HTMLInputElement>element).value };
            } else
            {
...
function GetStringWithLiterals(text: string, literalstr: string = "\"") {
    var result = {
        aliasedtext: text, literaldictionary: {}
    }
    var rs = literalstr + "(?:\\\\.|[^" + literalstr +"\\\\])*"+literalstr;
    var regex = new RegExp(rs, 'g');
    var matches: string[] = Coalesce(text.match(regex), []);
