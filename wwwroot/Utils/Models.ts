...
class ClientFilter implements IClientFilter {
     ...
     */
    public static CreateOr(children: IClientFilter[], fieldFormat: string = "upper({0})", type: string = "Text", field: string = "Id"): IClientFilter[] {
     ...
     * @returns The filter in a list or the original child in a list if there is only one child.
     */
    public static CreateAnd(children: IClientFilter[], fieldFormat: string = "upper({0})", type: string = "Text", field: string = "Id"): IClientFilter[] {
        children = children.filter(f => !IsNull(f))
        if (children.length == 0) {
            return null;
        } else if (children.length > 1) {
...
class ClientQuery {
    ...
    Ordering: Dictionary = {};
    Grouping: ClientQueryGroup = null;
    ...
    public CreateFilters(field: string, value: any, type: UIDataType = null): ClientFilter[] {
        var me = this;
        if (IsNull(type)) {
...
class IDB {
     ...
     */
    public Save(obj: object, storename: string, callback: Function, clearDB: boolean = true) {
        var me = this;
        var save = function () {
            if (!me.db.objectStoreNames.contains(storename)) {
    ...
    public DeleteData(storename: string, callback: Action<any[]>, filter: Func1<any, boolean> = null) {
        var me = this;
        var del = function () {
            var objectStore = me.GetStore(storename, 'readwrite')
            var result = [];
