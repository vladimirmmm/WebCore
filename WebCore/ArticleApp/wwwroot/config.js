...
var base_viewfiles = [
    "layout\\Settings.List.razor.html",
    "layout\\Settings.Login.razor.html",
    "layout\\BaseModel.List.razor.html",
    "layout\\BaseModel.Details.razor.html"

];
var otherfiles = [
    "configdata/resources-en.json",
    "configdata/resources-ro.json",
...
{
    var response = await fetch(appbaseurl+ "UI-version.js" + rkey, { credentials: 'include' });
    if (response.ok) {
...
for (var i = 0; i < files.length; i++) {
    if (!files[i].startsWith(appbaseurl)) {
        files[i] = appbaseurl + files[i];
    }
}
...
function loadresources() {
    if (typeof document !== 'undefined') {
        ...
        if ('serviceWorker' in navigator) {
            ...
            //    .register('PartnerAppSW.js?domain=' + appsettings.Domain, { scope: "/", files: files })
